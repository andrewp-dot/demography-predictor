...
class EvaluateModel(BaseEvaluation):
    ...
    #     AT_FIT_TIME_SEEN_FEATURES = FEATURES
    #     if set(FEATURES) != set(scaler.feature_names_in_):
    #         logger.warning(
    #             f"Unused features due to be unseen at the fit time: {set(FEATURES) - set(scaler.feature_names_in_)}"
    #         )

    #     AT_FIT_TIME_SEEN_FEATURES = FEATURES

    #     test_X = test_X[AT_FIT_TIME_SEEN_FEATURES]
    #     test_y = test_y[TARGETS]
    ...
    #     test_X = pd.DataFrame(test_X, columns=AT_FIT_TIME_SEEN_FEATURES)
    #     test_y = pd.DataFrame(test_y, columns=TARGETS)
    ...
    #     # Save true values
    #     self.reference_values = pd.DataFrame(test_y, columns=test_y.columns)

    #     logger.debug(f"[Eval]: predictions shape: {predictions.shape}")

    #     # Get metrics for time series model
    #     if FEATURES == TARGETS:
    #         self.get_feature_specific_metrics(features=AT_FIT_TIME_SEEN_FEATURES)
